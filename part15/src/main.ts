...
const inputFolder = 'part15/__input__';
...
function printGrid(grid: GridElement[][]) {
  ...
  for (let i = 0; i < grid.length; i++) {
    console.log(grid[i].map((element) => element.isAntiNode ? '#' : element.value).join(''));
  }
  console.log('=====================');
}
...
function setAntiNode(left: GridElement, right: GridElement) {
  ...
  if (colOffset > 0) {
    antiNodeColOffset = -Math.abs(colOffset) * 2;
  } else {
    antiNodeColOffset = Math.abs(colOffset) * 2;
  }
  ...
  if (rowOffset > 0) {
    antiNodeRowOffset = -Math.abs(rowOffset) * 2;
  } else {
    antiNodeRowOffset = Math.abs(rowOffset) * 2;
  }
  if (
    ...
    console.log('Set antinode', left.row + antiNodeRowOffset, left.col + antiNodeColOffset);
    grid[left.row + antiNodeRowOffset][left.col + antiNodeColOffset].isAntiNode = true;
  }
